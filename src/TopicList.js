...
function Topic(props) {
  if (props.selected) {
    return (
      <div onClick={props.onClick} style={{ width: 298, height: 60, textAlign: "left", backgroundColor: "#AAAAAA", border: 1, borderStyle: "solid" }}>
          <p style={{margin: 0, padding: 0, height: 30}}>{props.topic}</p>
          <p style={{margin: 0, padding: 0, height: 30}}>{props.type}</p>
      </div>
    )
  } else {
    return (
      <div onClick={props.onClick} style={{ width: 298, height: 60, textAlign: "left", backgroundColor: "#EEEEEE", border: 1, borderStyle: "solid" }}>
          <p style={{margin: 0, padding: 0, height: 30}}>{props.topic}</p>
          <p style={{margin: 0, padding: 0, height: 30}}>{props.type}</p>
      </div>
    )
